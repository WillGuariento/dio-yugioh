...
// init();
const state = {
  score: { playerScore: 0, computerScore: 0, scoreBox: document.getElementById("score_points") },
  cardDisplay: { avatar: document.getElementById("card-image"), name: document.getElementById("card-name"), type: document.getElementById("card-type") },
  fieldCards: { player: document.getElementById("player-field-card"), computer: document.getElementById("computer-field-card") },
  playerAreas: { player: { id: "player-cards", element: document.querySelector("#player-cards") }, computer: { id: "computer-cards", element: document.querySelector("#computer-cards") } },
  actions: { nextDuelButton: document.getElementById("next-duel") },
  audio: { bgm: document.getElementById("bgm"), effects: {} },
};

const PATH_IMAGES = "./src/assets/icons/";
const CARD_DATA = [
  { id: 0, name: "Blue Eyes White Dragon", type: "Paper", img: `${PATH_IMAGES}dragon.png`, winsAgainst: [1], losesTo: [2] },
  { id: 1, name: "Dark Magician", type: "Rock", img: `${PATH_IMAGES}magician.png`, winsAgainst: [2], losesTo: [0] },
  { id: 2, name: "Exodia", type: "Scissors", img: `${PATH_IMAGES}exodia.png`, winsAgainst: [0], losesTo: [1] },
];

// Preload de áudios
["win", "lose", "draw", "flip"].forEach(status => {
  const audio = new Audio(`./src/assets/audios/${status}.wav`);
  audio.volume = 0.2;
  state.audio.effects[status] = audio;
});

function getRandomCardId(excludeIds = []) {
  const available = CARD_DATA.map(c => c.id).filter(id => !excludeIds.includes(id));
  return available[Math.floor(Math.random() * available.length)];
}

function playAudio(status) {
  const audio = state.audio.effects[status];
  if (!audio) return;
  audio.currentTime = 0;
  audio.play().catch(err => console.error(`Error playing ${status}:`, err));
}
...
function createCardImage(cardId, fieldSide) {
  ...
  cardImage.classList.add("card", "card-back");

  if (fieldSide.id === state.playerAreas.player.id) {
    cardImage.addEventListener("mouseover", () => displaySelectedCardDetails(cardId));
    cardImage.addEventListener("click", () => handleCardSelection(cardId, cardImage));
  }
...
function removeAllCardsImages() {
  Object.values(state.playerAreas).forEach(area => area.element.querySelectorAll("img").forEach(img => img.remove()));
}
...
function drawCards(numberOfCards, fieldSide) {
  const usedIds = [];
  for (let i = 0; i < numberOfCards; i++) {
    const randomIdCard = getRandomCardId(usedIds);
    usedIds.push(randomIdCard);
    const cardImage = createCardImage(randomIdCard, fieldSide);
    fieldSide.element.appendChild(cardImage);
  }
}
...
function toggleFieldCardVisibility(show) {
  [state.fieldCards.player, state.fieldCards.computer].forEach(card => {
    card.style.display = show ? "block" : "none";
    if (show) card.style.opacity = 0; // Fade-in effect
  });
}
...
function displayDuelCards(playerCardId, computerCardId) {
  ...
  [playerCard, computerCard].forEach(card => { card.style.transition = "transform 0.6s"; card.style.transform = "rotateY(180deg)"; });
  
  setTimeout(() => {
    playerCard.src = CARD_DATA[playerCardId].img;
    computerCard.src = CARD_DATA[computerCardId].img;
    [playerCard, computerCard].forEach(card => card.style.transform = "rotateY(0deg)");
    [playerCard, computerCard].forEach(card => card.style.opacity = 1);
    playAudio("flip");
  }, 300);
}
...
function applyVisualEffect(status) {
  document.body.style.transition = "background-color 0.5s";
  const colors = { Win: "rgba(0, 255, 0, 0.3)", Lose: "rgba(255, 0, 0, 0.3)", Draw: "rgba(255, 255, 0, 0.3)" };
...
function checkDuelResults(playerCardId, computerCardId) {
  ...
  if (playerCard.winsAgainst.includes(computerCardId)) { duelResult = "Win"; state.score.playerScore++; }
  else if (playerCard.losesTo.includes(computerCardId)) { duelResult = "Lose"; state.score.computerScore++; }

  playAudio(duelResult.toLowerCase());
  applyVisualEffect(duelResult);
  return duelResult;
}

function handleCardSelection(cardId, clickedCard) {
  removeAllCardsImages();
  const computerCardId = getRandomCardId([cardId]);
  toggleFieldCardVisibility(true);
  hideCardDetails();
  displayDuelCards(cardId, computerCardId);
...
function init() {
  ...
  if (state.audio.bgm) {
    state.audio.bgm.volume = 0.1;
    state.audio.bgm.loop = true;
    state.audio.bgm.play().catch(() => console.warn("Autoplay bloqueado, clique para iniciar música."));
  }
}

// Listener do botão apenas uma vez
state.actions.nextDuelButton.addEventListener("click", resetDuel);
